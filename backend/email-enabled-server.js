...
const { sendPurchaseConfirmation } = require('./emailUtils');
...
app.get('/api/health', (req, res) => {
  if (mongoose.connection.readyState === 1) {
    ...
    const emailConfigured = EMAIL_HOST && EMAIL_PORT && EMAIL_USER && EMAIL_PASSWORD && EMAIL_FROM;
...
app.post('/api/email/test', async (req, res) => {
  try {
    ...
    // Send test email
    const emailSent = await sendPurchaseConfirmation(testTransaction);
