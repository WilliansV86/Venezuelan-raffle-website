@echo off
echo ===================================================
echo STARTING COMPLETE VENEZUELAN RAFFLE SOLUTION
echo ===================================================
echo.

echo 1. Stopping any existing Node.js processes...
taskkill /F /IM node.exe > nul 2>&1
timeout /t 2 > nul

echo 2. Starting integrated solution (API + admin + tickets)...
start cmd /k "cd %~dp0backend && node final-integrated-solution.js"
timeout /t 5 > nul

echo 3. Testing server connection...
curl http://localhost:5100/ping
if %errorlevel% neq 0 (
  echo ERROR: Server not responding! Check port conflicts.
  pause
  exit /b 1
)
echo.

echo 4. Starting React frontend...
start cmd /k "cd %~dp0frontend && npm start"

echo.
echo ===================================================
echo COMPLETE INTEGRATED SOLUTION RUNNING!
echo.
echo 1. Main website:     http://localhost:3000
echo 2. Admin dashboard:  http://localhost:3000/admin/payments
echo    (Login with: test-admin-key-123)
echo.
echo FULL PURCHASE AND ADMIN FLOW NOW WORKS:
echo - Select a raffle and click "Comprar Ticket"
echo - Fill out the form and upload a payment proof image
echo - Submit the form to create a transaction 
echo - Go to the admin panel to approve the payment
echo - Admin will see all transactions in the dashboard
echo - Admin can confirm payments and emails will be sent
echo.
echo ===================================================
